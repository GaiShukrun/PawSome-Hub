...
const CheckoutPage = ({ username }) => {
    ...
    const [formData, setFormData] = useState({
      ...
      const handleSubmit = async (e) => {
        e.preventDefault();
        if(flag){
        const response1 = await axios.get(`http://localhost:3001/api/checkout/${username}`);
      ...
      //2
        ...
        formDataWithItems.OrderDate = currentDate.toLocaleDateString('en-GB');
          ...
          if(window.confirm('Your Order has been placed successfully')){
            window.location.href="/";
          }
        } catch (error) {
        ...
        {items.map((item) => (
          <div className="item-box" key={item._id}>
           ...
           <div className="item-details">  
              ...
              <p>Quantity: {flag ? item.quantity : 1}</p>
            </div>
